...
export function PlanGantt({
  ...
  if (dated.length === 0) {
    return <EmptyState icon={AlertTriangle} title={ru.plan.unscheduled} description={ru.plan.noDates} />;
  }
      ...
      <div className="min-w-full">
        ...
        <div className="flex border-b border-border bg-muted/40">
          <div style={{ width: LABEL_W }} className="flex shrink-0 border-r border-border">
            ...
            <span className="px-3 py-1.5 text-xs font-medium text-muted-foreground">{ru.plan.taskTitle}</span>
          </div>
          <div className="grid flex-1" style={{ gridTemplateColumns: gridCols }}>
            {columns.map((c, i) => (
